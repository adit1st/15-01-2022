...
<div class=" mt-5 col-start-1 col-span-6 xl:col-start-2 xl:col-span-4">
  ...
  <div class="font-black col-start-1 self-center justify-self-start">
    ...
    @if ($isOpen)
      @include('livewire.create_comp')
    @endif
  </div>

  <div class="col-start-2 col-span-2 justify-self-start md:justify-self-end">
    <input class="rounded-full mb-3 w-11/12" type="text" wire:model="search" placeholder="Search Competition" aria-label="search">
  </div>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="text-center">
        <button wire:click="edit({{$c->id}})" class="mt-1 mb-1 bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-4 rounded-full">
          Edit
        </button>
        <button wire:click="openModalDelete({{$c->id}})" class="mt-1 mb-1 bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-4 rounded-full">
          Delete
        </button>
        @if ($deleteOpen)
    ...
    <div class="bg-red-400 mb-3 rounded">
        <h1 class="py-3 px-3 font-bold text-white">Competition Not Found</h1>
    </div>
    @endforelse
  </tbody>
</table>
